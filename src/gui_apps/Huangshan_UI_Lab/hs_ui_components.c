                                  ...
                                  hs_ui_status_t status)
{
    ...
    lv_obj_set_size(card, 112, 90);

    lv_obj_t *title_label = hs_ui_label_create(card, title, FONT_SMALL, HS_UI_COLOR_MUTED);
    lv_obj_align(title_label, LV_ALIGN_TOP_LEFT, 0, 0);

    lv_obj_t *value_label = hs_ui_label_create(card, value, FONT_BIGL, HS_UI_COLOR_TEXT);
    lv_obj_align(value_label, LV_ALIGN_BOTTOM_LEFT, 0, -4);

    lv_obj_t *unit_label = hs_ui_label_create(card, unit, FONT_SMALL, HS_UI_COLOR_MUTED);
    lv_obj_align_to(unit_label, value_label, LV_ALIGN_OUT_RIGHT_BOTTOM, HS_UI_SPACE_1, -2);
                                    ...
                                    hs_ui_status_t status)
{
    ...
    if (HS_UI_STATUS_WARN == status)
    {
        lv_obj_add_style(badge, &hs_ui_style_badge_warn, 0);
    }
    else if (HS_UI_STATUS_DANGER == status)
    {
        lv_obj_add_style(badge, &hs_ui_style_badge_danger, 0);
    }
    else
    {
        lv_obj_add_style(badge, &hs_ui_style_badge_ok, 0);
    }

    lv_obj_t *label = hs_ui_label_create(badge, text, FONT_SMALL,
                                         HS_UI_STATUS_WARN == status ? lv_color_hex(0x1F1604) :
                                         HS_UI_STATUS_DANGER == status ? HS_UI_COLOR_TEXT :
                                         HS_UI_COLOR_INK);
    lv_obj_center(label);
    return badge;
}
...
lv_obj_t *hs_ui_action_button_create(lv_obj_t *parent, const char *text, bool danger)
{
    ...
    lv_obj_set_size(btn, 108, 46);
    lv_obj_clear_flag(btn, LV_OBJ_FLAG_SCROLLABLE);

    lv_obj_t *label = hs_ui_label_create(btn, text, FONT_SMALL,
