...
- 在屏幕中部左右滑动切换当前监控到的任务；普通状态不显示底部切换按钮。出现受支持的
  审批请求时，底部临时显示 `Allow / Deny`。
...
当前可用的源宠物来自 Petdex 的 `Boba`、`Boxcat` 和 `Shinchan`。板端只安装一个 active 宠物；
当前 active 是 `Shinchan`，可用 `CODEX_PET_ACTIVE_SLUG=boxcat` 或 `--active=boxcat` 替换。
Petdex 源代码是 MIT，但宠物素材
...
板端的 active 宠物由 `petdex_pets.json` 的 `activeSlug` 决定；重新构建预载包即可替换，
不需要把其他宠物的帧安装到板子。启用 Huangshan MCP 后仍可在 Codex 中调用
`huangshan_select_pet`，但单 active 包不再提供运行时多宠物轮换。
...
替换一块全新板时，可以用下面的脚本一次完成固件刷写、Codex Pet 基础 App 安装和最终状态
查询。脚本使用本机已安装的 SiFli 工具链，不要求联网；默认构建也会自动选择离线工具链，
如需强制使用 SDK 的在线环境可设置 `SIFLI_SDK_FORCE_ONLINE=1`。
