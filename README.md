...
| Runtime App Manager / Launcher API | 已补第一版；串口 / BLE 可读取当前 App 状态、列出已安装 App、启动、停止和删除已停止 App；板端也有 App Manager 页面，支持分页查看 App、触摸启动、刷新和二次确认后删除已停止 App |
...
| 语音桥 `voice_start/status/read/clear` | 已跑通；串口 / BLE 都已完成 16 kHz PCM 短录音、WAV 落盘、回复回写与证据校验 |
...
| Runtime manifest + Lua subset | 已可用；支持基础组件、图片、按钮、定时 label、RGB、传感器 label、屏幕亮度/状态 label、语音状态/信息流 label、触摸状态 label、GPIO 状态 label |
...
| 串口 / BLE 可靠性回归脚本 | 已可用；安装、abort、冷启动恢复、`capabilities`、`sensors`、`power`、`display`、`touch`、`gpio`、`rgb`、`flow`、`voice-status` 和真实 `voice` 录音链路已能自动回归 |
...
| 方向 | 计划 |
| --- | --- |
| App 管理器 / 触摸 Launcher | 第一版命令/API 和板端原生 App Manager 已接入；当前已有分页和删除二次确认，下一步补滚动/图标、失败详情弹窗和更完整的长切换恢复 |
...
| 音频产品化 API | 当前录音桥、只读状态 JSON、App 受控录音触发和信息流回写链路已经打通；下一步补播放 / 回放状态接口，以及更长时稳定性 |
...
| 自动回归覆盖面 | 当前串口 / BLE 已覆盖安装、abort、冷恢复、`capabilities`、`sensors`、`power`、`display`、`touch`、`gpio`、`rgb`、`flow`、`voice-status` 和真实 `voice` 录音；下一步补更长时、多轮和跨端回归 |
...
| 方向 | 当前差距 | 下一步目标 |
| --- | --- | --- |
| App Manager / Launcher | 已有状态、列表、启动、停止、删除命令和板端可触摸分页页，但还缺滚动/图标和失败详情 | 继续打磨成可长期使用的 Launcher，补图标、滚动、失败详情和恢复提示 |
| Lua/LVGL App 能力 | 当前是受控 manifest + Lua subset，不是完整 Lua VM 和完整 LVGL binding | 先继续加高价值 binding；是否引入完整 Lua VM 需要单独评估 RAM、稳定性和安全边界 |
...
| 高阶能力 | 还没有 NES/native module、gamepad、复杂网络、相机、播放链路等 | Runtime API 稳定后再逐步引入，避免底座没稳就堆功能 |
...
产品底座。第四，受控 manifest/Lua subset 虽然能力不如完整 Lua VM，但更容易
做权限白名单、崩溃隔离和面向 AI 生成 App 的安全边界。
...
./scripts/runtime_reliability.sh /dev/cu.usbserial-13220 \
  ...
  --verify-flow \
  --verify-voice-status \
  --apps display_stage gpio_keys_stage power_stage touch_stage voice_stage
...
返回 JSON 一致；`--verify-flow` 会在同一轮里依次执行 `flow_clear`、`flow_send`、`flow_status`、`flow_clear`，确认信息流历史、计数和最新 seq/channel 都能闭环；`--verify-voice-status` 只读 `vb_runtime_voice` JSON，确认语音桥状态字段稳定且不会启动录音；`--verify-voice` 会跑一轮串口语音桥闭环，抓取板载麦克风短录音、
...
`runtime_full_reliability.sh` 默认会先校验选中的 Runtime App 包，再碰硬件；加 `--package-only` 可以只跑离线包校验，不需要串口参数。`runtime_full_reliability.sh --core` 会把串口和 BLE 两条链路按顺序跑完，适合做发布前 gate；当前核心 API gate 继续覆盖 `display_stage` / `gpio_keys_stage` / `power_stage` / `touch_stage` / `voice_stage`，其中串口和 BLE 都覆盖安装、`capabilities`、`sensors`、`power`、`display`、`touch`、`gpio`、`rgb`、`flow` 和只读 `voice-status`。不要同时并行跑串口和 BLE 安装回归，因为同一块板子的安装 session 是共享状态，并行访问会制造 `session=--` 或旧 status 误判这类伪故障。
...
执行；状态会显示 `lua=script-subset`。`vb_runtime_app` 返回当前 App Manager 状态 JSON，包含 active app、运行/失败状态、启动/停止计数和最近错误；`vb_runtime_apps` 返回已安装 App 列表，并标记 manifest/app.info/main.lua 是否存在、是否兼容和是否 active。`vb_runtime_launch <appId>` 会先校验 App 存在且兼容，再写入 active 并异步 reload；`vb_runtime_stop` 会停止当前 App 并回到板端 App Manager；`vb_runtime_delete <appId>` 只允许删除已停止的 App，避免用户误删正在运行的包。板端 Runtime 页面右上角 `Apps` 可打开 App Manager，当前支持分页查看、触摸启动、刷新，以及行内 `Del` -> `OK` 二次确认后删除已停止 App；图标、滚动和失败详情还在下一阶段。主机侧包校验会提前拒绝明显超出子集的 `function/for/while/if/require/dofile` 等语法，以及未知 helper 调用。当前 manifest 最低约束是
`kind=huangshan-runtime-app-manifest`、`id=<appId>`、`entry=main.lua`，以及
`schemaVersion`/`version` 为 `1`；manifest 组件当前会校验 `status/clock/action/label` 类型，以及 sensor/power/display/voice/touch/gpio/clock/status/reload 等已知能力白名单。若 manifest 声明 `runtimeProfile` / `targetProfile` / `target`，必须指向 `huangshan` / `huangshan-pi` / `vibeboard-huangshan` / `sf32*`；若声明顶层 `capabilities` / `requires` / `permissions`，打包器会按黄山派 profile 校验，并拒绝 `wifi`、`http`、`network`、`ntp`、`board_ip`、`native`、`camera`、`gamepad`、`i2s` 等 ESP32 或板载联网时代的能力。`weather.current` 这类能力表示手机/桌面 bridge 已经注入的数据，不表示板子自己联网；旧的 `app.info` 兼容入口仍然保留。这个子集先支持 `print(...)`、基础
...
时会显示 Runtime 记录到的触摸状态。Lua 子集也新增了
...
`vibe_voice_start("0..5000")` / `vibe_voice_clear()` 受控请求短录音和清空录音，
...
`vibe_touch_label(label, "count|last|event|gesture|delta|duration|active")`。
...
它声明当前固件支持的 Runtime API 版本、串口/BLE 安装、manifest/Lua 子集、
资源包、显示/触摸、传感器、语音桥、信息流、只读电池电压、AW32001
只读充电状态、屏幕亮度/面板快照、KEY 白名单 GPIO 和板载 RGB LED。当前 `battery` / `charger` 已同时开放 JSON、manifest status 和 `vibe_power_label(...)` App 侧展示路径；`display` 已开放 JSON、manifest status、`vibe_display_label(...)` 和 `vibe_display_brightness(...)` App 侧路径；`voice` 已开放只读 JSON、manifest status/action、`vibe_voice_label(...)`、`vibe_voice_start(...)` 和 `vibe_voice_clear()` App 侧路径；`flow` 已开放 host 写入、Runtime 历史、manifest status 和 `vibe_flow_label(...)` App 侧只读展示路径；`battery`、`charger`、`display`、`gpio` 和 `rgb`
...
`--voice-only`。这个接口只描述当前录音桥状态，不会启动麦克风采集。App 侧如需触发短录音，可以声明 manifest action `voice.start` / `voice.record`，其中 `value` 是毫秒数并会被 Runtime 限制到 0-5000 ms；`voice.clear` 会清空当前录音状态。Lua 子集也提供 `vibe_voice_start("1000")` 和 `vibe_voice_clear()`，但 App 仍不能直接读取 PCM，音频数据读取继续由串口/BLE 桥的 `voice_read` 完成。示例：
...
App 安装、`capabilities` 长 JSON 读取，以及 `sensors`、`power`、`display`、`touch`、`gpio`、`rgb`、`flow`、只读 `voice-status` 和真实 `voice` 录音链路
...
板端对应的 BLE/串口命令是 `voice_start <ms>`、`voice_status`、
`voice_read <offset> <maxBytes>`、`voice_clear`；返回文本复用
`flow_send pc.voice <seq> <hexUtf8Text>`。当前固件把录音作为
16 kHz、16-bit、mono PCM 分块拉回电脑，BLE 带宽适合短语音指令，不适合长时
实时音频流。
...
服务端会先跑同一套 Runtime 包校验，确认 manifest / `main.lua` / 路径白名单通过后再安装。页面启动时会先读 `/api/runtime/capabilities` 完成 Runtime 能力握手，并显示显示、触摸、传感器、语音、信息流、电源、GPIO、RGB 和安装通道摘要；页面同时暴露板上 App Manager：`/api/runtime/apps` 读取已安装 App，`/api/runtime/apps/<appId>/launch`、`/delete` 和 `/api/runtime/apps/stop` 通过同一个 serial/BLE adapter 发送管理命令；服务端用 transport lock 串行化 status、capabilities、install 和 App Manager 操作，避免浏览器轮询和安装/启动命令抢同一个连接：
