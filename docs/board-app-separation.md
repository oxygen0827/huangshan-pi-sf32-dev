...
黄山派现在还有一层更贴近产品的分离：Runtime 固件和 Runtime App 包分离。
固件一次烧录，负责 CO5300、FT6146、SD 卡、BLE/串口协议、RuntimeTransport
命令、能力白名单和 LVGL helper；用户生成的 App 放在
`/sdcard/apps/<app_id>`，通常只包含 `manifest.json`、`app.info`、`main.lua`
和 assets。

这层分离不是替代 SiFli 的 BSP/app 分层，而是在应用层内部再加一层：

```text
SiFli board/BSP 固定硬件事实
  -> huangshan-pi-sf32-dev 固件提供 Runtime 能力
  -> /sdcard/apps/<app_id> 提供可安装/删除/启动的用户 App
```

板端首页只做日常启动：扫描已安装 App，显示安全区卡片，点击后写入
`.active` 并进入 Runtime。安装、删除、刷新和错误解释由电脑 Web bridge 或
手机 BLE 工具完成。这样可以保持板上 UI 简洁，也避免把工程型 App Manager
页面暴露给 C 端用户。
