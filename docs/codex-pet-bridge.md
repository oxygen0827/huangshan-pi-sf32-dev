...
`TransportCommandQueue` 对普通 Runtime/GATT 命令使用 15 秒 deadline，关闭最多 8 秒；连接 deadline
由 BLE transport 按两轮实时扫描、服务发现和认证窗口动态计算（当前默认配置约 170 秒），
验证仍使用连接级上限。命令超时后队列立即拒绝已排队请求并进入恢复状态，只允许 `close` 和
...
板端只以任务快照的 `a=1` 加有效一次性审批 ID 判断“真实待审批”，不得通过详情文字中
出现 `approval` 推断审批。自动审批模式仍可能产生瞬时 `PermissionRequest` Hook，但此时
`a=0`，任务快照归一为 `running`，板端继续运行态动画，不显示 `Approval required` 或
`Allow / Deny`。真实审批只显示一行
`Approval required`，重复的详情行会隐藏，任务计数上移；后续 `PreToolUse`、`PostToolUse`
或新任务事件会清除已经由桌面处理的旧审批快照。
