...
- 执行受控 Lua subset 和 Runtime helper。
- 维护能力 JSON：display、sensor、power、touch、gpio、rgb、flow、voice 等。

## Host/iOS/Web 负责

- 选择 serial 或 BLE transport，并串行化对板命令。
- 离线校验包：App ID、路径、manifest、Lua subset、能力白名单。
...
- 只调用白名单 helper，例如 `vibe_label(...)`、`vibe_power_label(...)`、
  `vibe_flow_label(...)`、`vibe_2048_game(...)`。
...
- 完整 Lua VM/LVGL binding。
- native module、NES、camera、gamepad、I2S/audio out。
